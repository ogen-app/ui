...
| **P4** | **A date range on `GET /api/posts`** — `?from=&to=`, or a slim projection, or both. | The endpoint takes no parameters and returns the tenant's posts *hydrated* — Markdown bodies, campaign, platform and asset graphs — which is what the workspace calendar is drawn from (`useWorkspacePosts`) however few weeks are on screen. Nobody has raised it because nothing was hurt by it yet: `useAssetUsage` and the auto-publish allowlist have read the same list for months and both say in their own comments that it is fine at today's scale. A calendar makes it a hot path rather than a column. The client's half is one query key and a param, and the grid already knows the range it is showing. Until then the flag stays off — not because the screen doesn't work, but because "opens instantly" is the only thing a calendar is for. |
