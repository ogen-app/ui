  ...
  insights: [
    {
      id: 'rate_vs_reach',
      severity: 'info',
      text: 'Reach grew faster than interactions.',
    },
  ],
}
...
describe('fetchAnalyticsOverview', () => {
  it('asks for a relative window and unwraps the envelope', async () => {
    ...
    expect(fetchMock.mock.calls[0][0]).toBe(
      '/api/analytics/overview?window=28d',
    )
  ...
  it('reads a workspace with nothing published as a state, not a failure', async () => {
    stubFetch(
      jsonResponse(200, { available: false, reason: 'no_data', data: null }),
    )
  ...
  it("surfaces a rejected range as the server's code", async () => {
    ...
    await expect(
      fetchAnalyticsOverview({ from: '2026-08-19', to: '2026-07-22' }),
