...
export function ShortsPreview({ text, title, media, author, timeLabel }: PreviewProps) {
  ...
  // A Short's caption is its title. YouTube falls back to the first line of
  // the description, then to "Untitled Video" — the same chain the watch page
  // previews, because it is the same field.
  const caption = title.trim() || firstLine(text) || 'Untitled Video'
