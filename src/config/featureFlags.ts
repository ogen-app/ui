...
const FEATURE_FLAGS = {
   ...
   * a rhythm on its own Foundation page, which is where all the work happens. A
   * campaign may also define one locally and promote it once it proves it
   * recurs, so the library fills from use instead of being seeded with four
   * generic nouns at onboarding.
   *
   * Strategy prints the arithmetic and owns none of it: rhythms are set per
   * series, and the mix is derived from them against `postGoalTotal`. There is
   * deliberately no percentage picker — a mix you type is a plan you break in
   * week two, and a mix you derive is a fact.
   *
