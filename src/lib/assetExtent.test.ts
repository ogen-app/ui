...
describe("extentLabel", () => {
  ...
  it("reads an empty processing asset as a wait, not a verdict", () => {
    expect(extentLabel({ content: "", status: "processing" })).toBe("Not read yet");
  });

  it("names the failure a size never would: uploaded fine, extracted to nothing", () => {
    expect(extentLabel({ content: "", status: "partial" })).toBe("Nothing extracted");
    expect(extentLabel({ content: "", status: "ready" })).toBe("Nothing extracted");
  });
});
