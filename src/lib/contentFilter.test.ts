...
const asset = (title: string, status: AssetStatus = 'ready', tags: Tag[] = []): Asset =>
  ({
    id: title,
    title,
    status,
...
const is = (facet: FilterFacet, id: string): Clause => ({ facet, id, negated: false })
const not = (facet: FilterFacet, id: string): Clause => ({ facet, id, negated: true })

const filter = (clauses: Clause[], name = ''): ContentFilter => ({ name, clauses })
...
describe('filterAssets', () => {
  ...
  it('treats two values of one facet as any-of', () => {
    expect(
      titles(filterAssets(library, filter([is('status', 'processing'), is('status', 'failed')]))),
    ).toEqual(['Market sizing working document', 'LinkedIn post archive'])
  })

  it('treats two tags as any-of, not all-of', () => {
    expect(filterAssets(library, filter([is('tag', 't1'), is('tag', 't2')]))).toHaveLength(3)
  })

  it('ands separate facets, and the name, together', () => {
    expect(
      titles(filterAssets(library, filter([is('status', 'ready'), is('tag', 't1')], 'e'))),
...
describe('filterAssets, negated', () => {
  ...
  it('ands two exclusions rather than or-ing them', () => {
    ...
    expect(titles(filterAssets(library, filter([not('tag', 't1'), not('tag', 't2')])))).toEqual([
      'Market sizing working document',
    ])
  })

  it('subtracts from what the wanted values allowed', () => {
    const found = filterAssets(library, filter([is('tag', 't1'), not('status', 'failed')]))
    expect(titles(found)).toEqual(['Words we don’t use'])
  })

  it('can empty the list by excluding everything present', () => {
    const clauses = ['ready', 'processing', 'failed'].map((id) => not('status', id))
...
describe('parseModifier', () => {
  it('reads a keyword and the value being typed after it', () => {
    expect(parseModifier('status:re')).toEqual({ facet: 'status', query: 're', negated: false })
    expect(parseModifier('tag:')).toEqual({ facet: 'tag', query: '', negated: false })
  })
  ...
  it('reads a leading minus as an exclusion', () => {
    expect(parseModifier('-tag:legal')).toEqual({ facet: 'tag', query: 'legal', negated: true })
  })
...
describe('withClause / withoutClause', () => {
  it('adds in the order values were reached', () => {
    const two = withClause(withClause(EMPTY_FILTER, 'tag', 't2', false), 'status', 'ready', false)
  ...
  it('flips a value in place rather than standing it beside itself', () => {
    ...
    expect(withClause(one, 'tag', 't1', true).clauses).toEqual([not('tag', 't1')])
  })

  it('removes only the value named', () => {
    const two = filter([is('status', 'ready'), not('status', 'failed')])
    expect(withoutClause(two, 'status', 'ready').clauses).toEqual([not('status', 'failed')])
  })
})
...
describe('filterChips', () => {
  it('gives one chip per clause, in order, labelled as it is shown', () => {
    ...
    expect(chips).toEqual([
      { facet: 'status', id: 'ready', negated: false, keyword: 'status', label: 'Ready' },
      { facet: 'tag', id: 't1', negated: true, keyword: 'tag', label: 'Brand voice' },
    ])
  })
})
...
describe('statusVocabulary', () => {
  it('offers only the statuses present, in life-cycle order', () => {
    expect(statusVocabulary(library).map((v) => v.id)).toEqual(['processing', 'ready', 'failed'])
  })
...
describe('suggest', () => {
  ...
  it('drops the including half once the minus is typed', () => {
    expect(suggest('-tag:legal', EMPTY_FILTER, vocab)).toEqual([
      { kind: 'value', facet: 'tag', keyword: 'tag', negated: true, id: 't2', label: 'Legal' },
    ])
  })
  ...
  it('never offers a value already claimed, whichever way it was claimed', () => {
    ...
    expect(found.map((s) => (s.kind === 'value' ? s.id : s.kind))).toEqual(['t2', 't2'])
  })

  it('stops offering a modifier with nothing left to give', () => {
    const spent = filter([is('tag', 't1'), not('tag', 't2')])
    expect(suggest('', spent, vocab).map((s) => s.keyword)).toEqual(['status', 'status'])
  })
  ...
  it('caps prose at six values and keeps every pair whole', () => {
    const found = suggest('e', EMPTY_FILTER, vocab).filter((s) => s.kind === 'value')
