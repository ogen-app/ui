 ...
 *
 * Neither field is written from here any more. Since CON-233 the campaign's
 * documents are changed through the membership endpoints, which derive
 * `use_assets` from the set they leave behind — so the client no longer has to
 * spell an empty set as `use_assets: false` to keep the server from reading it
 * as everything. See `lib/campaignMembership`.
 */
