...
describe('setupChecks', () => {
  ...
  it('fails when the connected channels have no post type selected', () => {
    ...
    expect(channelReadiness(campaign, views).missingPostTypes).toEqual([
      'LinkedIn',
    ])
    const check = channelsCheck(campaign, views)
    expect(check.ok).toBe(false)
...
function ids(campaign: Campaign, posts: Post[], views = connected): string[] {
...
describe('attentionItems', () => {
  ...
  it('is empty for a healthy mid-campaign', () => {
    expect(
      attentionItems(liveCampaign(), healthyPosts(), connected, NOW),
    ).toEqual([])
  })
  ...
  it('separates a manual publish that is due from an auto slot that was missed', () => {
    ...
    const auto = attentionItems(
      ...
      [
        ...healthyPosts(),
        makePost({ status: 'scheduled', scheduled_at: iso(-1 * HOUR) }),
      ],
      connected,
      NOW,
  ...
  it('flags a post planned inside 24h that is still a draft, but not one planned later', () => {
    ...
    const later = [
      ...healthyPosts(),
      makePost({ status: 'draft', scheduled_at: iso(3 * DAY) }),
    ]
    expect(ids(liveCampaign(), later)).not.toContain(
      'planned-today-unscheduled',
    )
  })
  ...
  it('flags two posts crowding the same slot on one channel', () => {
    const posts = [
      ...
      makePost({
        status: 'scheduled',
        scheduled_at: iso(3 * DAY + 5 * MINUTE),
      }),
    ]
  ...
  it('escalates a missing account to an alert once posts are scheduled against it', () => {
    ...
    expect(
      blocking.find((i) => i.id === 'accounts-missing-blocking'),
    ).toMatchObject({
      severity: 'alert',
    })
  })
  ...
  it('says nothing about an unconnected channel while another one can publish', () => {
    ...
    expect(ids(campaign, healthyPosts(), views)).not.toContain(
  ...
  it('flags a connected channel whose accounts are all inactive, and stays quiet when the payload is silent', () => {
    ...
    expect(ids(liveCampaign(), healthyPosts(), inactive)).toContain(
      'account-inactive',
    )
    // Empty `accounts` means "not stated", not "inactive".
    expect(ids(liveCampaign(), healthyPosts())).not.toContain(
      'account-inactive',
    )
  })
  ...
  function droppedLinkedIn(): Campaign {
    return liveCampaign({
      target_platforms: [{ id: 'p2', post_types: ['text-post'] }],
    })
  }
  ...
  it('flags posts scheduled outside the campaign dates', () => {
    const posts = [
      ...healthyPosts(),
      makePost({ status: 'scheduled', scheduled_at: '2026-09-20T10:00:00Z' }),
    ]
  ...
  it('treats the end date as inclusive — the final day is still in-window', () => {
    ...
    const posts = [
      ...healthyPosts(),
      makePost({ status: 'scheduled', scheduled_at: '2026-08-31T18:00:00Z' }),
    ]
  ...
  it('says nothing about phases for a campaign type that has none', () => {
    ...
    const posts = healthyPosts().map((p) => ({
      ...p,
      campaign_type_phase_id: 'gone',
    }))
  ...
  it("says nothing about the campaign's post target, set or not", () => {
    ...
    expect(
      ids(liveCampaign({ estimated_post_count: null }), healthyPosts()),
    ).not.toContain('post-target')
  })
  ...
  it('flags a connected channel with no posts, and stays quiet about an unconnected one', () => {
    ...
    expect(ids(campaign, healthyPosts(), offline)).not.toContain(
      'channel-uncovered',
    )
  })
